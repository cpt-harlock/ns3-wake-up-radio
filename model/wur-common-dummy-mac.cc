...
void WurCommonDummyMac::OnDataRx(Ptr<Packet> packet) {
	...
	NS_LOG_INFO("WurCommonDummyMacHeader: " << macHeader.GetFrom() << " " << macHeader.GetTo());
}
...
Address WurCommonDummyMac::GetAddress() const { return m_address; }

void WurCommonDummyMac::SetPromisc() {}

void WurCommonDummyMac::StartDataTx() {
	if (m_mainRadioPhy->GetState() ==
	    WurMainRadioNetDevicePhyStateHelper::OFF)
		m_mainRadioPhy->TurnOn();
	if (m_mainRadioPhy->GetState() ==
	    WurMainRadioNetDevicePhyStateHelper::IDLE) {
		Ptr<Packet> packet = m_txqueue.front();
		m_txqueue.erase(m_txqueue.begin());
		NS_LOG_INFO(" WurCommonDummyMac::StartDataTx m_txqueue size " +
			    std::to_string(m_txqueue.size()));
