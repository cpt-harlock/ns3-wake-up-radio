...
#include "radio-interference-helper.h"
#include "error-rate-model.h"
#include "wifi-utils.h"

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("RadioInterferenceHelper");

/****************************************************************
 *       PHY event class
 ****************************************************************/

RadioEvent::RadioEvent (Ptr<const WurCommonPpdu> ppdu, Time duration, double rxPower)
  : m_ppdu (ppdu),
...
Ptr<const WurCommonPsdu>
RadioEvent::GetPsdu (void) const
{
  return m_ppdu->GetPsdu ();
}

Ptr<const WurCommonPpdu>
RadioEvent::GetPpdu (void) const
{
  return m_ppdu;
}

Time
RadioEvent::GetStartTime (void) const
{
  return m_startTime;
}

Time
RadioEvent::GetEndTime (void) const
{
  return m_endTime;
}

Time
RadioEvent::GetDuration (void) const
{
  return m_endTime - m_startTime;
}

double
RadioEvent::GetRxPowerW (void) const
{
  return m_rxPowerW;
}
...
RadioInterferenceHelper::NiChange::NiChange (double power, Ptr<RadioEvent> event)
  : m_power (power),
    m_event (event)
{
}
...
double
RadioInterferenceHelper::NiChange::GetPower (void) const
{
  return m_power;
}

void
RadioInterferenceHelper::NiChange::AddPower (double power)
{
  m_power += power;
}

Ptr<RadioEvent>
RadioInterferenceHelper::NiChange::GetRadioEvent (void) const
{
  return m_event;
}
...
RadioInterferenceHelper::RadioInterferenceHelper ()
  : m_errorRateModel (0),
    m_numRxAntennas (1),
    m_firstPower (0),
    m_rxing (false)
{
  ...
  AddNiChangeRadioEvent (Time (0), NiChange (0.0, 0));
}
...
Ptr<RadioEvent>
RadioInterferenceHelper::Add (Ptr<const WurCommonPpdu> ppdu,  Time duration, double rxPowerW)
{
  Ptr<RadioEvent> event = Create<RadioEvent> (ppdu,  duration, rxPowerW);
  AppendRadioEvent (event);
  return event;
}

void
RadioInterferenceHelper::AddForeignSignal (Time duration, double rxPowerW)
{
...
void
RadioInterferenceHelper::SetNoiseFigure (double value)
{
  m_noiseFigure = value;
}

void
RadioInterferenceHelper::SetErrorRateModel (const Ptr<ErrorRateModel> rate)
{
  m_errorRateModel = rate;
}

Ptr<ErrorRateModel>
RadioInterferenceHelper::GetErrorRateModel (void) const
{
  return m_errorRateModel;
}

void
RadioInterferenceHelper::SetNumberOfReceiveAntennas (uint8_t rx)
{
  m_numRxAntennas = rx;
}

Time
RadioInterferenceHelper::GetEnergyDuration (double energyW) const
{
...
void
RadioInterferenceHelper::AppendRadioEvent (Ptr<RadioEvent> event)
{
...
double
RadioInterferenceHelper::CalculateSnr (double signal, double noiseInterference, WifiTxVector txVector) const
{
...
double
RadioInterferenceHelper::CalculateNoiseInterferenceW (Ptr<RadioEvent> event, NiChanges *ni) const
{
...
double
RadioInterferenceHelper::CalculateChunkSuccessRate (double snir, Time duration, WifiMode mode, WifiTxVector txVector) const
{
  if (duration.IsZero ())
    {
      return 1.0;
...
double
RadioInterferenceHelper::CalculatePayloadChunkSuccessRate (double snir, Time duration, WifiTxVector txVector) const
{
  if (duration.IsZero ())
    {
      return 1.0;
...
double
RadioInterferenceHelper::CalculatePayloadPer (Ptr<const Event> event, NiChanges *ni, std::pair<Time, Time> window) const
{
...
double
RadioInterferenceHelper::CalculateNonHtPhyHeaderPer (Ptr<const Event> event, NiChanges *ni) const
{
  NS_LOG_FUNCTION (this);
  const WifiTxVector txVector = event->GetTxVector ();
  double psr = 1.0; /* Packet Success Rate */
...
double
RadioInterferenceHelper::CalculateHtPhyHeaderPer (Ptr<const Event> event, NiChanges *ni) const
{
  NS_LOG_FUNCTION (this);
  const WifiTxVector txVector = event->GetTxVector ();
  double psr = 1.0; /* Packet Success Rate */
...
struct RadioInterferenceHelper::SnrPer
RadioInterferenceHelper::CalculatePayloadSnrPer (Ptr<RadioEvent> event, std::pair<Time, Time> relativeMpduStartStop) const
{
  NiChanges ni;
...
double
RadioInterferenceHelper::CalculateSnr (Ptr<RadioEvent> event) const
{
  NiChanges ni;
  double noiseInterferenceW = CalculateNoiseInterferenceW (event, &ni);
  double snr = CalculateSnr (event->GetRxPowerW (),
...
struct RadioInterferenceHelper::SnrPer
RadioInterferenceHelper::CalculateNonHtPhyHeaderSnrPer (Ptr<RadioEvent> event) const
{
  NiChanges ni;
  double noiseInterferenceW = CalculateNoiseInterferenceW (event, &ni);
  double snr = CalculateSnr (event->GetRxPowerW (),
...
struct RadioInterferenceHelper::SnrPer
RadioInterferenceHelper::CalculateHtPhyHeaderSnrPer (Ptr<RadioEvent> event) const
{
  NiChanges ni;
  double noiseInterferenceW = CalculateNoiseInterferenceW (event, &ni);
  double snr = CalculateSnr (event->GetRxPowerW (),
...
void
RadioInterferenceHelper::EraseEvents (void)
{
  m_niChanges.clear ();
  // Always have a zero power noise event in the list
...
RadioInterferenceHelper::NiChanges::const_iterator
RadioInterferenceHelper::GetNextPosition (Time moment) const
{
  return m_niChanges.upper_bound (moment);
}

RadioInterferenceHelper::NiChanges::const_iterator
RadioInterferenceHelper::GetPreviousPosition (Time moment) const
{
...
RadioInterferenceHelper::NiChanges::iterator
RadioInterferenceHelper::AddNiChangeEvent (Time moment, NiChange change)
{
  return m_niChanges.insert (GetNextPosition (moment), std::make_pair (moment, change));
}

void
RadioInterferenceHelper::NotifyRxStart ()
{
  NS_LOG_FUNCTION (this);
  m_rxing = true;
}
...
void
RadioInterferenceHelper::NotifyRxEnd ()
{
  NS_LOG_FUNCTION (this);
  m_rxing = false;
